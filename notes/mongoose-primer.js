...
// orFail() method makes a query throw an error if no document matches the given filter
// useful because it saves you an extra if statement to check if no document was found

// if you don't specify a callback then the API will return a query object which you
// can save to a variable and execute later
// you can narrow down the query by chaining  the where() method or using sort(), select()
// or limit() methods on the query variable

// .find() returns all matching docs but you can also use findbyId(), findOne(), etc

// You can create references from one document/model instance to another using the 
// ObjectId schema field, or from one document to many using an array of ObjectIds. 
// The field stores the id of the related model. If you need the actual content of
// the associated document, you can use the populate() method in a query to replace 
// the id with the actual data.
...
var authorSchema = Schema({
  name    : String,
  stories : [{ type: Schema.Types.ObjectId, ref: 'Story' }] // here ref tells the schema which model it's referring to with the object id
});
...
// to create those relationships between models, assign the dependent model a
// property with the value <parentModel>._id
